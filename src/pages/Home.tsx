...
import { Globe, Zap, Database, Bot, Lock, ArrowRight, Sparkles, Wand2, MessageSquare, Shield, Users, MapPin, CreditCard } from 'lucide-react';
...
const aiFeatures = [
  ...
  },
  {
    ...
    description: 'Multi-turn intelligent chat grounded in real-time digital tides.',
    icon: Sparkles,
    path: '/surfer',
...
export const Home = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4">
      ...
      {/* Hero Promo Video Framework */}
      <div className="mb-32 relative">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, duration: 0.8 }}
          className="relative max-w-5xl mx-auto aspect-video rounded-[3rem] overflow-hidden glass-card border border-white/10 group shadow-[0_0_100px_rgba(0,255,255,0.1)]"
        >
          {/* Replace this div with an actual <video src="..." /> tag when your video is ready */}
          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-center p-8">
             <div className="w-24 h-24 bg-neon-cyan/20 rounded-full flex items-center justify-center border border-neon-cyan/30 mb-8 backdrop-blur-md animate-pulse">
                <Globe className="text-neon-cyan" size={40} />
             </div>
             <h3 className="text-3xl md:text-5xl font-black italic tracking-tighter text-white mb-4">THE OCEAN TIDE DROP</h3>
             <p className="text-neon-cyan text-sm font-black tracking-[0.3em] uppercase mb-6 drop-shadow-[0_0_8px_rgba(0,255,255,0.8)]">Ride the Informational Waves</p>
             <p className="text-slate-400 text-lg font-light max-w-xl">
               Add your AI Surfer promo video here. Show the world what you are building, the digital assets you are selling, and how the Collective accelerates growth.
             </p>
          </div>
        </motion.div>
      </div>

      {/* Collective Membership Section */}
      <div className="mb-24">
        <div className="flex items-center gap-3 mb-8 px-4">
          <Shield className="text-neon-cyan" size={20} />
          <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-500">Collective Membership</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            className="group relative p-10 glass-card rounded-[2.5rem] border border-neon-cyan/20 bg-neon-cyan/5 overflow-hidden"
          >
            <div className="absolute -top-12 -right-12 text-neon-cyan/10 group-hover:text-neon-cyan/20 transition-colors">
              <Shield size={200} />
            </div>
            <div className="relative z-10">
              <h3 className="text-4xl font-black italic mb-4 uppercase tracking-tighter text-neon-cyan">The Sanctuary</h3>
              <p className="text-slate-400 text-lg font-light leading-relaxed mb-8 max-w-sm">
                Access the inner circle. Neural synthesis tools, private chat protocols, and proprietary Hatteras data nodes.
              </p>
              <Link 
                to="/sanctuary"
                className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-white hover:text-neon-cyan transition-colors"
              >
          ...
          <motion.div
            ...
            className="group relative p-10 glass-card rounded-[2.5rem] border border-white/10 hover:border-white/20 overflow-hidden"
          >
            <div className="absolute -top-12 -right-12 text-white/5 group-hover:text-white/10 transition-colors">
              <CreditCard size={200} />
            </div>
            <div className="relative z-10">
              <h3 className="text-4xl font-black italic mb-4 uppercase tracking-tighter text-white">Neural Plans</h3>
              <p className="text-slate-400 text-lg font-light leading-relaxed mb-8 max-w-sm">
                Scalable intelligence gates for every architect. From community nodes to enterprise automation.
              </p>
              <Link 
                to="/pricing"
                className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-white hover:text-neon-cyan transition-colors"
