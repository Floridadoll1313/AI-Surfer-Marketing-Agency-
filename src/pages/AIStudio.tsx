...
type Persona = 'Architect' | 'Visionary' | 'Specialist' | 'AI Surfer' | 'Custom';
...
const personas = {
  ...
  "AI Surfer": "You are the AI Surfer of the Hatteras Collective. You ride the informational waves of the digital ocean. Your language is laid-back, surf-inspired, and focused on navigating the fluid currents of data.",
...
export const AIStudio = () => {
    ...
    try {
      ...
      if (manifestType === 'image') {
        ...
        if (part) {
          ...
          setResultUrl(url);
          setHistory(prev => [{ id: Date.now().toString(), tab: 'manifest', type: 'image', prompt, url, timestamp: new Date() }, ...prev]);
        }
        ...
        if (uri) {
          try {
            ...
            setResultUrl(url);
            setHistory(prev => [{ id: Date.now().toString(), tab: 'manifest', type: 'video', prompt, url, timestamp: new Date() }, ...prev]);
          } catch (fetchErr) {
        ...
        if (audioBase64) {
          ...
          const url = URL.createObjectURL(blob);
          setResultUrl(url);
          setHistory(prev => [{ id: Date.now().toString(), tab: 'manifest', type: 'music', prompt, url, timestamp: new Date() }, ...prev]);
        }
      }
  ...
  const handleObserve = async () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const textResult = response.text || "Neural core returned empty analysis.";
        setAnalysisResult(textResult);
        setHistory(prev => [{ id: Date.now().toString(), tab: 'observe', type: observeType, prompt: prompt || 'Analysis', text: textResult, timestamp: new Date() }, ...prev]);
      };
  ...
  const transcribeAudio = async (blob: Blob) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const textResult = response.text || "No acoustic data manifested.";
        setTranscription(textResult);
        setHistory(prev => [{ id: Date.now().toString(), tab: 'sonic', type: 'transcription', prompt: 'Audio transcription', text: textResult, timestamp: new Date() }, ...prev]);
      };
  ...
  const handleTTS = async () => {
    ...
    try {
      ...
      if (part) {
        ...
        const url = URL.createObjectURL(blob);
        setResultUrl(url);
        setHistory(prev => [{ id: Date.now().toString(), tab: 'sonic', type: 'tts', prompt, url, timestamp: new Date() }, ...prev]);
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-[1600px] w-full mx-auto py-12 px-4 lg:px-8 flex flex-col min-h-screen">
      {/* Immersive Header */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-center justify-between mb-8 pb-8 border-b border-white/10 gap-6">
        <div className="flex items-center gap-6">
          <div className="p-4 bg-neon-cyan/10 rounded-3xl border border-neon-cyan/20 relative group shrink-0">
            <Wand2 className="text-neon-cyan animate-pulse group-hover:scale-110 transition-transform" size={32} />
            <div className="absolute inset-0 bg-neon-cyan/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
          <div>
            <h1 className="text-4xl md:text-5xl font-black italic tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white via-neon-cyan to-white uppercase leading-none mb-2">
              Neural Labs
            </h1>
        ...
        <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-full flex items-center gap-3">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">API Link: {hasApiKey ? 'Stable' : 'Offline'}</span>
        </div>
      </motion.div>

      <div className="flex flex-col lg:flex-row gap-6 flex-1 items-start">
        {/* Left Sidebar: Lab Navigation */}
        <div className="w-full lg:w-48 shrink-0 flex flex-row lg:flex-col gap-2 overflow-x-auto no-scrollbar pb-2 lg:pb-0">
          {[
            { id: 'manifest', label: 'Manifest', icon: Sparkles, color: 'text-neon-cyan', bg: 'bg-neon-cyan/10', border: 'border-neon-cyan/50' },
            { id: 'observe', label: 'Observe', icon: Eye, color: 'text-neon-pink', bg: 'bg-neon-pink/10', border: 'border-neon-pink/50' },
            { id: 'dialogue', label: 'Dialogue', icon: MessageSquare, color: 'text-neon-green', bg: 'bg-neon-green/10', border: 'border-neon-green/50' },
            { id: 'sonic', label: 'Sonic', icon: Volume2, color: 'text-neon-yellow', bg: 'bg-neon-yellow/10', border: 'border-neon-yellow/50' },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id as LabTab); setResultUrl(null); }}
              className={cn(
                "flex flex-row lg:flex-col items-center lg:items-start gap-3 p-4 rounded-3xl transition-all relative overflow-hidden group border",
                activeTab === tab.id ? `${tab.bg} ${tab.color} border-white/20` : "bg-white/5 border-white/5 text-slate-500 hover:text-white hover:bg-white/10"
              )}
            >
              <tab.icon size={24} className={cn(activeTab === tab.id ? "animate-pulse" : "")} />
              <span className="text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap">
                {tab.label}
              </span>
              {activeTab === tab.id && <motion.div layoutId="sidebar-active" className={cn("absolute left-0 top-0 bottom-0 w-1 rounded-r-full", tab.bg.replace('/10', ''))} />}
            </button>
          ))}
        </div>

        {/* Middle Column: Configuration Matrix */}
          ...
          <div className="glass-card p-8 rounded-[3rem] border border-white/10 relative overflow-hidden">
            ...
            <header className="flex items-center justify-between mb-8">
              <h3 className="text-sm font-black uppercase tracking-[0.3em] flex items-center gap-3 text-slate-400">
                <Brain className="text-neon-cyan" size={16} /> Parameters
              </h3>
            </header>
              ...
              {activeTab === 'dialogue' && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['Architect', 'Visionary', 'Specialist', 'AI Surfer', 'Custom'] as Persona[]).map(p => (
                      <button
                        key={p}
        ...
        <div className="flex-1 flex flex-col gap-6 min-w-0">
          ...
          {history.length > 0 && activeTab !== 'dialogue' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 glass-card p-6 rounded-3xl border border-white/10">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-6 flex items-center gap-2">
                <Layers size={14} /> Output Archive
              </h4>
              <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                {history.map((record) => (
                  <div key={record.id} className="relative w-48 shrink-0 bg-black/40 rounded-2xl border border-white/5 overflow-hidden group">
                    {record.type === 'image' && record.url ? (
                      <img src={record.url} alt={record.prompt} className="w-full h-32 object-cover opacity-70 group-hover:opacity-100 transition-opacity" />
                    ) : record.type === 'video' && record.url ? (
                      <video src={record.url} className="w-full h-32 object-cover opacity-70 group-hover:opacity-100 transition-opacity" />
                    ) : (
                      <div className="w-full h-32 flex items-center justify-center text-slate-600 bg-white/5">
                        {record.type === 'music' || record.type === 'tts' || record.type === 'transcription' ? <Music size={32} /> : <Eye size={32} />}
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent p-4 flex flex-col justify-end">
                      <p className="text-[9px] font-bold text-white line-clamp-2 leading-snug">{record.prompt}</p>
                      <span className="text-[8px] text-neon-cyan uppercase tracking-widest mt-1">{record.type}</span>
                    </div>
                  </div>
                ))}
              </div>
