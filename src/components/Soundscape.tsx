...
import { Volume2, VolumeX, Music, Loader2, Sparkles, Shield } from 'lucide-react';
...
export const Soundscape = () => {
  ...
  const handleOpenSelectKey = async () => {
    ...
    setHasApiKey(true);
    // Proceed to generate if it was already "playing"
    if (isPlaying) {
      generateSoundscape();
    }
  };
  ...
  const generateSoundscape = async () => {
    // Check if key is selected for Lyria models
    const selected = await (window as any).aistudio?.hasSelectedApiKey();
    if (!selected) {
      setIsPlaying(false);
      return;
    ...
    try {
      ...
      if (!key) {
        console.warn('API key not found, soundscape generation skipped.');
        return;
      }
      ...
      <div className="flex items-center gap-3">
        {!hasApiKey && isPlaying ? (
          <button
            ...
            className="flex items-center gap-2 px-4 py-3 bg-neon-purple text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-[0_0_20px_rgba(123,47,247,0.5)] hover:bg-neon-purple/80 transition-all"
          >
            <Shield size={14} />
            Unlock Neural Audio (Select API Key)
          </button>
        ) : (
          <button
            onClick={togglePlay}
